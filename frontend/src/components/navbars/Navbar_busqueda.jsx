                    ...
                    >
                        ...
                        >
                            ...
                            <ul className="ml-10 mt-[100] space-y-6">
                                ...
                                {isAdmin &&
                                    Object.keys(adminOptions[language]).map((key) => (
                                        <li key={key}>
                                            <Link href={`/general/${key}_general`}>
                                                <motion.span
                                                    className="InterRegular font-bold text-black-500 cursor-pointer hover:text-gray-600 transition duration-300 block"
                                                    whileHover={{ scale: 1.1 }}
                                                >
                                                    {adminOptions[language][key]}
                                                </motion.span>
                                            </Link>
                                        </li>
                                    ))}
            ...
            <AnimatePresence>
                {loginOpen && (
                    <motion.div
                        ...
                        transition={{ duration: 0.5, ease: "easeInOut" }}
                    >
                        <LoginForm language={language} onClose={() => setLoginOpen(false)} onLoginSuccess={handleLoginSuccess} />
                    </motion.div>
                )}
            </AnimatePresence>
            ...
            <AnimatePresence>
                {logoutOpen && (
                    <motion.div
                        ...
                        transition={{ duration: 0.5, ease: "easeInOut" }}
                    >
                        <LogoutModal language={language} onClose={() => setLogoutOpen(false)} onLogoutConfirm={handleLogout} />
                    </motion.div>
                )}
            </AnimatePresence>
        </nav>
